    ...
    >
      <div className="flex w-14 flex-col items-center rounded-e-xl border-r border-slate-600 bg-slate-700">
        ...
        <button
          ...
          className={`absolute -right-5 top-1/2 h-11 w-11 ${toggleBar ? "translate-x-0" : "translate-x-3"} -translate-y-[55%] rounded-full bg-slate-600 transition-transform duration-300`}
        >
          <i className={`fa fa-chevron-right ${toggleBar ? "rotate-180" : "rotate-0"} items-center text-center text-xl font-bold text-slate-50 transition-all duration-300 ease-linear`}></i>
        </button>
        <nav className="flex max-h-full flex-col gap-y-4 pt-4">
          ...
          <a href="#" className="text-gary-400 group relative rounded-xl p-2 hover:bg-slate-600">
            <svg width="24" height="24" className="h-6 w-6 stroke-current group-hover:text-cyan-600" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
        ...
        <div className="flex flex-col items-center gap-y-4 pb-4 pt-12">
          ...
          <button className="mt-2 rounded-full bg-gray-100">
            <img className="h-10 w-10 rounded-full" src="/logo.jpg" alt="Avatar" />
          </button>
        </div>
      </div>
    </aside>
