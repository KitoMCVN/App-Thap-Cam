...
const ReactImage: React.FC<ReactImageProps> = ({ src, alt, fallback, loader, className }) => {
  ...
  if (loading) {
    return <div className="size-full bg-slate-500 animate-pulse">{loader || ""}</div>;
  }

  if (error) {
    return <div className="size-full bg-red-300">{fallback || "Image failed to load."}</div>;
  }
...
export const Image: React.FC<ReactImageProps> = (props) => (
  <Suspense fallback={<div className="size-full">{props.loader || "Loading..."}</div>}>
    <ReactImage {...props} />
  </Suspense>
);
